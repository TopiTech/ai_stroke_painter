...
bool Node::paintStroke(const QList<QPointF> points, const QList<qreal> pressures, const QString strokeStyle)
{
    if (points.size() < 2 || points.size() != pressures.size()) {
        dbgScript << "Script passed invalid point/pressure arrays to Node::paintStroke(), ignoring.";
        return false;
    }

    for (qsizetype index = 0; index < points.size(); ++index) {
        const QPointF point = points.at(index);
        const qreal pressure = pressures.at(index);
        if (!std::isfinite(point.x()) || !std::isfinite(point.y()) || !std::isfinite(pressure) ||
            pressure < 0.0 || pressure > 1.0) {
            dbgScript << "Script passed a non-finite coordinate or invalid pressure to Node::paintStroke(), ignoring.";
            return false;
        }
    }
    ...
    KisFigurePaintingToolHelper helper = PaintingResources::createHelper(d->image, node(), strokeStyle);
    KisPaintInformation previousPoint;
    previousPoint.setPressure(pressures.first());
    previousPoint.setPos(points.first());

    for (qsizetype index = 1; index < points.size(); ++index) {
        KisPaintInformation currentPoint;
        currentPoint.setPressure(pressures.at(index));
        currentPoint.setPos(points.at(index));
        helper.paintLine(previousPoint, currentPoint);
