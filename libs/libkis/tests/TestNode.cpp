...
void TestNode::testPaintStrokeRejectsInvalidInput()
{
    KisImageSP image = new KisImage(0, 100, 100, KoColorSpaceRegistry::instance()->rgb8(), "test");
    KisNodeSP layer = new KisPaintLayer(image, "paint stroke target", 255);
    NodeSP node = NodeSP(Node::createNode(image, layer));

    const QList<QPointF> points = {QPointF(1.25, 2.5), QPointF(10.75, 20.5)};
    QVERIFY(!node->paintStroke(points, {0.5}));
    QVERIFY(!node->paintStroke({points.first()}, {0.5}));
    QVERIFY(!node->paintStroke(points, {0.5, 1.5}));
}
