...
public Q_SLOTS:
    ...
    /**
     * @brief paintStroke paints a pressure-aware freehand stroke using the current brush preset.
     *
     * Unlike repeated calls to paintLine(), all segments are submitted through one Krita
     * freehand stroke. This preserves brush spacing, timing-independent interpolation and
     * pressure continuity across point boundaries.
     *
     * @param points ordered canvas positions; at least two points are required
     * @param pressures one pressure value in the 0.0 to 1.0 range for every point
     * @param strokeStyle appearance of the stroke; see paintLine()
     * @return true when the complete stroke was accepted, false when the input or node is invalid
     */
    bool paintStroke(const QList<QPointF> points,
