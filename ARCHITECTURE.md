...
- The composed plan records a semantic manifest, rendered elements, missing elements, node bounds, selected featured counts, the declarative foundation value plan, and the exact compiled gradient gamut. `quality.py` recomputes semantic fidelity, actual raster-footprint coverage, subject/background luminance-distribution separation, and effect intrusion from the post-budget result, so centerline coverage or coincident mean values cannot make a weak plan pass.
...
- `quality_check.py`: deterministic representative-scene gates for raster appearance, semantic fidelity, layer separation, bounds, brush/palette contracts, and generation time.
