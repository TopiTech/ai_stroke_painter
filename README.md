...
- **API キー**: 既定では現在のリクエストにだけ使用するキーです。Windows では明示的に保存を選んだ場合のみ、現在のユーザーと端末に紐づく DPAPI 保護で保存できます
...
エンドポイントとモデル名は次回起動のためローカル設定に保存されます。API キーは既定で
保存されず、リクエスト開始後に入力欄から消去されます。Windows の明示的な保存操作では
DPAPI により現在のユーザーと端末に紐づけて保護します。
...
API キーは既定では設定ファイルやレイヤーへ保存しません。Windows の明示的な保存を選んだ
場合も、設定には現在のユーザーと端末に紐づく DPAPI 保護済みデータだけを保存します。
画像モデル API を使う場合は入力プロンプトとキーが指定したプロバイダーへ送信されるため、
組織のポリシーに従い、機密情報をプロンプトへ入力しないでください。

このリポジトリには Krita 由来のコンポーネントが含まれます。各ファイルの著作権表示と
ライセンスヘッダーを維持しています。AI Stroke Painter の新規追加モジュール
（`libs/ui/aiillustration/*`）は `GPL-2.0-or-later` の下で作成されていますが、
Krita 上流の `GPL-3.0-or-later` コンポーネントと統合・リンクされたリポジトリ全体および
配布バイナリには、GNU General Public License v3 or later（**GPL-3.0-or-later**）が
適用されます（詳細はルート直下の [COPYING](COPYING) を参照してください）。
