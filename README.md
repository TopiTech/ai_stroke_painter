...
2. **🖼️ 参照画像の読み込み & 画像解析設定 (Image-to-Stroke)**
   - ローカル画像（PNG, JPG, WebP, BMP）を読み込み、連結輪郭トレース・陰影ハッチング・カラーサンプリングにより、完全オフラインで自然な手描きストロークへ自動変換。元画像の縦横比と透過領域を維持します。
...
以下のワンコマンドですべての静的解析（Ruff / Mypy）、通常環境の回帰テスト、Qt を読み込まないヘッドレス互換テストを実行できます：
