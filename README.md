...
- **LLM 座標ストローク描画**: OpenAI Chat Completions 互換 API (`/v1/chat/completions`) から LLM が座標指定したストロークを、Centripetal Catmull-Rom スプライン曲線補間・筆圧入り抜き（Tapering）・下地クリッピングマスクを用いてレイヤー別（Flats, Shading, Lineart, Highlights, FX）に高品質自動描画
...
- **ストローク予算**: 生成するストロークの本数上限（既定値: 500本）

LLM から返された `StrokeProgram` JSON に基づき、`Flats` (下塗り), `Shading` (陰影・乗算), `Lineart` (線画), `Highlights` (ハイライト・加算), `FX` (効果) の各レイヤーが自動作成され、筆圧やグラデーションを伴うストロークがキャンバスへ直接レンダリングされます。
