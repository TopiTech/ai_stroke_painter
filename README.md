...
3. **🖌️ ブラシ・描画 & レイヤートランザクション**
   - **失敗・停止時の自動ロールバック**: マルチ/単一レイヤー出力は実行専用コンテナだけを除去し、アクティブレイヤー直接描画は描画前の画素スナップショットを同じレイヤーへ復元します。標準Krita APIで安全性を担保し、環境固有のUndoマクロには依存しません。
   - **描画スケーリング**: 太さ倍率（0.1x〜5.0x） & 不透明度（10%〜100%）
   - **レイヤー出力モード**:
     - *マルチレイヤー分割*: 実行ごとに一意な出力グループを作り、Draft, Flats, Shading, Lineart, Highlights, FX 各レイヤーへ自動分割
     - *アクティブレイヤー*: 選択中のペイントレイヤーへ直接描画（失敗・停止時は描画前の画素へ自動復元）
...
- `stroke_program.py`: 正規化座標の StrokeProgram v2、v1移行、決定論的コンパイラ
- `brushes.py`: 全生成経路で共有する意味的ブラシ登録
...
- `krita_adapter.py`: キャンバスキャプチャ、マルチレイヤー管理、画素スナップショット式ロールバック、カラー・筆圧描画
...
- `quality.py`: 被覆率・レイヤー・筆圧・断片化・境界・描画コストの品質評価
...
代表6題の決定性・被覆・構造・生成時間ゲートは、プロジェクトの親ディレクトリから次で実行できます：
