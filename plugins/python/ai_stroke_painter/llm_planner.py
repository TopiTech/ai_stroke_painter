...
class OpenAICompatibleSettings:
    ...
    def __post_init__(self) -> None:
        ...
        if any(ord(char) < 32 or ord(char) == 127 for char in self.api_key):
            raise ValueError("API Key に制御文字を含めないでください")
...
class OpenAICompatiblePlanner(PlannerPort):
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                if attempt < max_attempts:
                    ...
                    self._log(f"エラー: 全 {max_attempts} 回の試行が失敗しました: {err_str}")
    ...
    def _post(
        ...
        headers = {
            ...
            "User-Agent": f"AIStrokePainter/{PLUGIN_VERSION}",
        }
...
def _system_instruction(
    ...
    if any(
        ...
        domain_guidance = (
            ...
            "   - Lower Lash & Tear Trough: 1-2 delicate discrete lower eyelash flicks under outer iris (size_px: 1.2 to 1.8 px, color: #221220).\n"
            ...
            "   - Lip Gloss Multi-Point Glint: Lustrous central specular sheen + subtle moisture glints on lower lip curve (size_px: 1.8 to 2.8 px, color: #ffffff).\n"
            ...
            "   - Hair Angel Halo (Ring of Light): Dynamic zig-zag tiered highlight clumps following hair volume (#ffffff / #fff2b2, size_px: 4 to 12 px, opacity: 0.75-0.95).\n"
...
def _sanitize_and_rescue_program_dict(
    ...
    for idx, item in enumerate(raw_ops):
        ...
        try:
            raw_opacity = b_dict.get("opacity")
            if raw_opacity is None:
                raw_opacity = item_d.get("opacity")
            op_opacity = float(raw_opacity) if raw_opacity is not None else 1.0
        except (ValueError, TypeError):
            op_opacity = 1.0
