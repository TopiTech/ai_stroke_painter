    ...
    def render(
        ...
        **kwargs: Any,
        ...
        self.last_render_trace = {
            ...
            "routes": {"native_stroke": 0, "continuous_path": 0, "segmented_line": 0},
            "resolved_presets": {},
        }
        ...
        try:
            ...
            for stroke in render_strokes:
                ...
                if not callable(getattr(current_node, "paintStroke", None)) and not hasattr(current_node, "paintLine"):
                    raise RuntimeError(
                        "このKritaには Node.paintStroke / Node.paintLine がありません。統合版Kritaを使用してください。"
                    )
                ...
                paint_stroke = getattr(current_node, "paintStroke", None)
                if callable(paint_stroke):
                    ...
                    native_points = list(stroke.points)
                    if len(native_points) == 1:
                        point = native_points[0]
                        native_points.append(StrokePoint(point.x + 0.5, point.y, point.pressure, point.time_ms + 1))
                    accepted = paint_stroke(
                        [_qpoint_float(point.x, point.y) for point in native_points],
                        [float(point.pressure) for point in native_points],
                    )
                    if accepted is False:
                        raise RuntimeError("Kritaが連続筆圧ストロークを拒否しました")
                    mutated = True
                    rendered += 1
                    self.last_render_trace["routes"]["native_stroke"] += 1
                    segment_count += max(1, len(native_points) - 1)
                ...
                if not hasattr(current_node, "paintLine"):
                    raise RuntimeError("このKritaには Node.paintLine がありません。統合版Kritaを使用してください。")
