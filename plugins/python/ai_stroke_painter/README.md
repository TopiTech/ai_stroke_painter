...
- このリポジトリに含まれる **Krita 6.1 fork**
- Krita の Python scripting support を有効にしたビルド

---

## ビルドと起動

AI Stroke Painter は `plugins/python/ai_stroke_painter` に統合され、通常の Krita ビルドと同時にインストールされます。配布 ZIP の作成や「Pythonプラグインをインポート」は不要です。

1. Krita の通常の依存関係を用意し、このプロジェクトルートを CMake で構成・ビルド・インストールします。
2. ビルドした Krita を初回起動すると AI Stroke Painter は既定で有効になります。
3. **設定 > ドッキングパネル > AI Stroke Painter** を表示します。

ユーザーが Python プラグインマネージャで明示的に無効化した設定は、次回起動でも維持されます。
...
- `ports.py`: 拡張境界ポート定義
- `kritalibkis Node.paintStroke`: 浮動小数点座標と点ごとの筆圧を、1本のKritaネイティブ・フリーハンドストロークとして描画
...
Krita 側の `libs/libkis/tests/TestNode` は連続ストロークAPIの入力検証を確認し、Python 側の回帰テストは全座標・全筆圧が1回の `paintStroke` 呼び出しへ渡ることを確認します。
