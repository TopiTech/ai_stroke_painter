...
class CanvasAdapterTests(unittest.TestCase):
    ...
    def test_native_stroke_receives_all_points_and_pressures(self) -> None:
        class NativeStrokeNode(_FakeNode):
            def __init__(self) -> None:
                super().__init__("target")
                self.strokes: list[tuple[list[Any], list[float]]] = []

            def paintStroke(self, points: list[Any], pressures: list[float]) -> bool:  # noqa: N802
                self.strokes.append((points, pressures))
                self.pixels = b"native stroke pixels"
                return True

        target = NativeStrokeNode()
        document = _FakeDocument(active=target)
        stroke = Stroke(
            "continuous",
            [
        ...
        plan = DrawingPlan("native", 1, [stroke], canvas_width=40, canvas_height=30)
        adapter = KritaCanvasAdapter(layer_mode="active_layer")
        ...
        self.assertEqual(target.strokes[0][1], [0.1, 0.8, 0.6, 0.1])
        self.assertEqual(len(target.strokes[0][0]), 4)
        self.assertEqual(adapter.last_render_trace["routes"]["native_stroke"], 1)
        self.assertEqual(apply_style.call_args.kwargs["size_multiplier"], 2.0)
        self.assertEqual(apply_style.call_args.kwargs["opacity_multiplier"], 0.5)
...
class CodeReviewFinalQualityTests(unittest.TestCase):
    ...
    def test_version_and_fingerprint_consistency(self) -> None:
        """統合版のバージョンとソースフィンガープリントが決定論的であること。"""
