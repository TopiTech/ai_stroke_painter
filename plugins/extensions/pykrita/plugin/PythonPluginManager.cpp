...
void PythonPluginManager::scanPlugins()
{
    ...
    Q_FOREACH(const QString &desktopFile, desktopFiles) {
        ...
        if (dg.readEntry("ServiceTypes") == "Krita/PythonPlugin") {
            ...
            const bool enabledByDefault = dg.readEntry("X-Krita-EnabledByDefault", false);
            plugin.m_enabled = pluginSettings.readEntry(QString("enable_") + plugin.moduleName(), enabledByDefault);
