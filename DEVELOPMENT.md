...
- API キーをログ、レイヤー名、クラッシュレポートへ書き込まない。
- API キーはデフォルトでは端末保存せず、Windows の明示的なオプトイン（「API キーをこの端末に保存する」）時のみ DPAPI で現在のユーザーと端末に紐づけて保護してからローカル設定に保持する。旧版の可逆 Base64 値は次回起動時に移行し、移行できない場合は削除する。保存チェックを外した際は即座にローカル設定から完全消去し、リクエスト後に入力欄をクリアする。
- エンドポイントとモデルは生成モード（LLM / 画像モデル）ごとに独立してローカル設定へ保存する。
